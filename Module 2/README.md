# GenAI - FastAPI LLM Backend with RAG (Module 2)

This FastAPI app forwards a user `question` to a Hugging Face chat model and returns structured JSON including latency, token estimates, intent/priority classification, and suggested support actions.

## 🆕 RAG Features (NEW)

**This version includes Retrieval-Augmented Generation (RAG):**
- Upload your own documents to a vector database
- Questions automatically retrieve relevant context
- LLM uses your documents for more accurate answers
- Optional RAG can be disabled per request
- Tracks RAG usage and document retrieval in metrics

## Features

- **Vector Database**: Chroma for semantic search
- **Document Upload**: Store and retrieve knowledge base
- **Smart Context**: Automatically injects relevant docs
- **Flexible**: Works with or without RAG
- **Metrics**: Detailed tracking including RAG stats

It also records basic metrics to a `metrics.csv` file in the same folder.
