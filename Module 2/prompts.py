...
def get_analysis_prompt(question: str, context: str = "") -> str:
    """
    ...
    Args:
        ...
        context: Optional RAG context from vector database
    ...
    return f"""{system_context}You are an AI assistant that analyzes user questions and classifies them.
...
def validate_analysis_response(response: Dict[str, Any]) -> bool:
    """
    Validate that the analysis response has required fields and valid values.
    ...
    """
    required_fields = {"answer", "intent", "priority", "confidence"}
    ...
    if response.get("intent") not in ["incident", "support", "information"]:
        return False
    
    if response.get("priority") not in ["critical", "high", "medium", "low"]:
        return False
